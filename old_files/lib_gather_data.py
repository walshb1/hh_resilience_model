...
def match_quintiles(hhdataframe,quintiles,sort_handle):

    hhdataframe.loc[hhdataframe[sort_handle]<=quintiles[0],'quintile']=1

    for j in np.arange(1,len(quintiles)):
        hhdataframe.loc[(hhdataframe[sort_handle]<=quintiles[j])&(hhdataframe[sort_handle]>quintiles[j-1]),'quintile']=j+1
