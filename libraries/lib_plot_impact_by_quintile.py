...
def plot_impact_by_quintile(myCountry,aProv,aDis,anRP,iah,my_PDS='no'):
    iah = iah.loc[iah.affected_cat=='a']
    ...
    for myQ in range(1,6): #nQuintiles
        ...
        k = 1E-3*(0.01*iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['k','pcwgt_no']].prod(axis=1).sum()/
                  iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_no'].sum())

        dk = 1E-3*(iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['dk0','pcwgt_no']].prod(axis=1).sum()/
                   iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_no'].sum())

        dc = 1E-3*(iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['dc_pre_reco','pcwgt_no']].prod(axis=1).sum()/
                   iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_no'].sum())

        dw = 1E-3*(iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['dw_no','pcwgt_no']].prod(axis=1).sum()/
                   iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_no'].sum())

        pds_nrh = 1E-3*(iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['net_help_received_'+my_PDS,'pcwgt_'+my_PDS]].prod(axis=1).sum()/
                        iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_'+my_PDS].sum())

        pds_dw =  1E-3*(iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),['dw_'+my_PDS,'pcwgt_'+my_PDS]].prod(axis=1).sum()/
                        iah.loc[(iah[economy]==aProv)&(iah.hazard==aDis)&(iah.rp==anRP)&(iah.quintile==myQ),'pcwgt_'+my_PDS].sum())
    ...
    plt.ylabel('Disaster losses [,000 '+get_currency(myCountry)[0][-3:]+' per affected person]',labelpad=8,fontsize=8)
    ...
    while not _success:
        try:
            fig.savefig(output_plots+'npr_'+aProv.replace(' ','')+'_'+aDis+'_'+str(anRP)+'_'+my_PDS+'.pdf',format='pdf',bbox_inches='tight')
            fig.savefig(output_plots+'png/npr_'+aProv.replace(' ','')+'_'+aDis+'_'+str(anRP)+'_'+my_PDS+'.png',format='png',bbox_inches='tight')
            _success = True
        except: 
