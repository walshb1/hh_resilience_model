...
def plot_income_and_consumption_distributions(myCountry,iah,aReg,aDis,anRP):
    ...
    for _fom,_fom_lab in [('i','Income'),
        ...
        net_chg_pov = round(net_chg_pov_i,-2)
        if _fom == 'c': net_chg_pov = round(net_chg_pov_c,-2)
        ...
        ax.annotate(int_w_commas(net_chg_pov)+'('+str(net_chg_pov_pct)+'%) more Filipinos\n face '+_fom_lab.lower()+' poverty',
                    weight='light',color=greys_pal[7],xy=(sf_x*1.1*iah.pov_line.mean(),1.14*cf_heights[:-2].max()),
        ...
        if sub_line is not None:
            ...
            ax.annotate(int_w_commas(net_chg_sub)+'('+str(net_chg_sub_pct)+'%) more Filipinos\n face '+_fom_lab.lower()+' subsistence',
                        weight='light',color=greys_pal[7],xy=(sf_x*1.1*sub_line,1.34*cf_heights[:-2].max()),
                        xycoords='data',ha='left',va='top',fontsize=9,annotation_clip=False)
