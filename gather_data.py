...
if not os.path.exists(inputs): #if the depository directory doesn't exist, create one
...
PSA = pd.read_excel(inputs+'PSA_compiled.xlsx',skiprows=1)[['province','gdp_pc_pp','pop','shew']].set_index('province')
...
# listofquintiles=np.arange(0.05, 1.01, 0.05) #create a list of quintiles 
# cat_info = match_quintiles(cat_info,perc_with_spline(reshape_data(cat_info.c),reshape_data(cat_info.weight),listofquintiles))
# cat_info['c_5_nat'] = cat_info.ix[cat_info.quintile==1,'c'].max()
# cat_info.drop('quintile',axis=1,inplace=True)
# listofquintiles=np.arange(0.2, 1.1, 0.2) #create a list of quintiles 
# cat_info = match_quintiles(cat_info,perc_with_spline(reshape_data(cat_info.c),reshape_data(cat_info.weight),listofquintiles))
# cat_info = cat_info.rename(columns={'quintile':'quintile_nat'}).reset_index()

cat_info = cat_info.rename(columns={'district':'code'})#change the name: district to code, and create an multi-level index 

listofquintiles=np.arange(0.20, 1.01, 0.20) #create a list of quintiles
...
#print(cat_info.loc[cat_info.quintile > 8]['quintile'].head(100))
#cat_info.drop('quintile',axis=1,inplace=True)
cat_info = cat_info.reset_index().set_index(['province','hhid'])

# listofquintiles=np.arange(0.2, 1.1, 0.2) #create a list of quintiles 
# cat_info = cat_info.groupby('district',sort=True).apply(lambda x:match_quintiles_score(x,perc_with_spline(reshape_data(x.score),reshape_data(x.weight),listofquintiles)))
...
# listofquintiles=np.arange(0.2, 1.1, 0.2) #create a list of quintiles 
# cat_info = cat_info.groupby('district',sort=True).apply(lambda x:match_quintiles(x,perc_with_spline(reshape_data(x.c),reshape_data(x.weight),listofquintiles)))
...
# This gives you the population of each province
# cat_info['n']*cat_info.weight.sum(level=economy)).sum(level=economy)
...
cat_info['shew'] = broadcast_simple(PSA['shew'],cat_info.index())
