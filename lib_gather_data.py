...
def match_deciles(hhdataframe,deciles):
	hhdataframe.loc[hhdataframe['c']<=deciles[0],'decile']=1
	for j in np.arange(1,len(deciles)):
		hhdataframe.loc[(hhdataframe['c']<=deciles[j])&(hhdataframe['c']>deciles[j-1]),'decile']=j+1
	return hhdataframe
	
def match_deciles_score(hhdataframe,deciles):
	hhdataframe.loc[hhdataframe['score']<=deciles[0],'decile_score']=1
	for j in np.arange(1,len(deciles)):
		hhdataframe.loc[(hhdataframe['score']<=deciles[j])&(hhdataframe['score']>deciles[j-1]),'decile_score']=j+1
	return hhdataframe
